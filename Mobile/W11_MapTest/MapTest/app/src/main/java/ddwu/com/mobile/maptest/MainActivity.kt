...
class MainActivity : AppCompatActivity() {
        ...
        override fun onMapReady(map: GoogleMap) {
            ...
            locClient.lastLocation.addOnSuccessListener { location: Location? ->
                if (location != null) {
                    ...
                    googleMap.moveCamera(
                        CameraUpdateFactory.newLatLngZoom( myLoc, 17f )
                    )
                    myMarker(myLoc)
                }
            }
            locClient.lastLocation.addOnFailureListener {
                myLoc = LatLng(37.606320, 127.041808)
                googleMap.moveCamera(
    ...
    val locRequest = LocationRequest.Builder(3000)
    ...
    private fun addMarker( latLng : LatLng ) {
        val markerOptions = MarkerOptions().apply {
            ...
            snippet("addresses.get(0).getAddressLine(0)")  // Marker Window 설명
        ...
        currentMarker?.tag = "DB_id"

        geocoder.getFromLocation(latLng.latitude, latLng.longitude, 1) {
            addresses ->
//            currentMarker?.snippet = addresses.get(0).getAddressLine(0)
            Log.d(TAG, addresses[0].toString() )
        }
    }

    private fun myMarker( latLng : LatLng ) {
        val markerOptions = MarkerOptions().apply {
            position(latLng)
            title("Marker Title")
